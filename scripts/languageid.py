...
ISO_CLD3_DICT = {'eng':'en', 'sqi':'sq', 'bos':'bs', 'ell':'el', 'mkd':'mk', 'srp':'sr', 'ukr':'uk', 
                'hye':'hy', 'kat':'ka', 'rus':'ru', 'uzb':'uz', 'ben':'bn', 'pus':'ps', 'urd':'ur', 
                'mya':'my', 'ind':'id', 'khm':'km', 'kor':'ko', 'lao':'lo', 'tha':'th', 'vie':'vi', 
                'amh':'am', 'bam':'fr', 'fra':'fr', 'hau':'ha', 'por':'pt', 'sna':'sn', 'som':'so', 
                'swh':'sw', 'fas':'fa', 'kur':'ku', 'tur':'tr', 'hat':'ht', 'spa':'es'}
...
def language(path: str, outdir: str) -> None:
    with open(path) as file:
        ...
        with open(tsv_name, "a", newline='') as tsv_file:
            fieldnames = ['file_name', 'url', 'is_supported', 'paragraph#', 'num_chars', 'language1', 'probability1', 'unexpected1', 'language2', 'probability2', 'unexpected2']
            writer = csv.DictWriter(tsv_file, fieldnames=fieldnames, restval='', extrasaction='raise')
            ...
            for i, prediction in enumerate(cld3.get_frequent_languages(text, num_langs=2)):
                prediction_dictionary[-1]['language' + str(i+1)] = prediction.language
                prediction_dictionary[-1]['probability' + str(i+1)] = prediction.probability
                prediction_dictionary[-1]['unexpected' + str(i+1)] = is_unexpected(prediction.language, iso)
            ...
            for i, paragraph in enumerate(paragraphs):
                if len(paragraphs) > 1:
                    ...
                    for j, prediction in enumerate(cld3.get_frequent_languages(paragraph, num_langs=2)):
                        prediction_dictionary[i]['language' + str(j+1)] = prediction.language
                        prediction_dictionary[i]['probability' + str(j+1)] = prediction.probability
                        prediction_dictionary[i]['unexpected' + str(j+1)] = is_unexpected(prediction.language, iso)
...
def run():
    ...
    parser.add_argument("inputdir", help="Input directory containing json in directories by iso/sitename")
    parser.add_argument("outdir", help="Output directory containing tsv by iso/sitename")
